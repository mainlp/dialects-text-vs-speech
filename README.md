...
- `xsid-audio`: Contains the mapping of all xSID dev/test sentences (CC BY-SA 4.0) to intents and to the audio files in https://zenodo.org/records/19554428. Except for `xsid_de-ba_valid.tsv`, these files are identical to the metadata files on Zenodo. **xsid_de-ba_valid.tsv fixes an issue with misaligned German/Bavarian sentences (and Bavarian sentences/audios) in the Zenodo metadata files.** This issue does not affect our experiments.
